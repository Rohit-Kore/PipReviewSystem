 ...
 /*  //by rohit old
  ...
  @Override
    public ResponseEntity<?> registerEmployee(Employee employee) {
        ...
        // ✅ Send welcome email
        mailService.sendMail(saved.getEmail(), "Welcome to PIP Review System",
...
*/
    ...
    public ResponseEntity<?> registerEmployee(Employee employee, String creatorRole) {
        ...
        // ✅ Enforce: Nobody can create an ADMIN through API
        if (Role.ADMIN.name().equalsIgnoreCase(employee.getRole().name())) {
            return new ResponseEntity<>("Creating an ADMIN is not allowed.", HttpStatus.FORBIDDEN);
        }

        // ✅ You can also check HR cannot create MANAGER if needed, add logic here
...
//    }
    ...
    public ResponseEntity<?> forgotPassword(String email, String newPassword) {
        ...
        mailService.sendMail(email, "Password Changed",
